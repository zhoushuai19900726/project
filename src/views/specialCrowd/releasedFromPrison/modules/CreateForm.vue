          ...
          <a-row>
            ...
            >
              <a-form-item label="危险性评估类型">
                <a-select
                  v-decorator="['riskAssessmentType']"
                  placeholder="请选择"
                  default-value="0"
                >
            ...
            >
              <a-form-item label="未安置原因">
                <a-input
                  v-decorator="['nonResettlementReasons']"
                  style="width: 100%"
                />
              </a-form-item>
            </a-col>
...
import { searchArchiveManagement, editArchiveManagement, editGovernReleasePrisoners } from '@/api/manage'
const fields = [
  // 'description',
  'id',
  'idCard',
    ...
    submitSpecial () {
      ...
      form.validateFields((errors, values) => {
        ...
        if (!errors) {
          ...
          return editGovernReleasePrisoners(data).then((res) => {
            // 重置表单数据
            form.resetFields()
            form1.resetFields()
            if (res.code === 200) {
