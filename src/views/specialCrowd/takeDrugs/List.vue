...
<script>
  ...
  import { getRoleList, editArchiveManagement, getAddress, getGovernDrugAddicts, deleteGovernDrugAddicts } from '@/api/manage'
  ...
  export default {
    ...
    data () {
      ...
      return {
        ...
        loadData: (parameter) => {
          ...
          return getGovernDrugAddicts(requestParameters).then((res) => {
            console.log(res)
            return res.result
          })
        },
    ...
    methods: {
      ...
      handleDel (record) {
        ...
        var id = record.id
        var arr = [id]
        return deleteGovernDrugAddicts(arr).then((res) => {
          console.log(res)
          if (res.code === 200) {
