          ...
          <a-row>
            ...
            >
              <a-form-item label="银行存折账号">
                <a-input
                  v-decorator="['bankPassbookAccount']"
                  placeholder="请输入"
                />
              </a-form-item>
            </a-col>
...
import { editThreeNonElderly } from '@/api/actualPopulation'
// 表单字段
const fields = [
  // 'description',
  'id',
    ...
    submitSpecial () {
      ...
      form.validateFields((errors, values) => {
        ...
        if (!errors) {
          ...
          return editThreeNonElderly(data).then((res) => {
            // 重置表单数据
            form.resetFields()
            if (res.code === 200) {
              form.resetFields()
