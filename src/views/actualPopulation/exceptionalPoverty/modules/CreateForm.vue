            ...
            >
              <a-form-item label="供养标准">
                <a-select
                  placeholder="请选择"
                  :disabled="openType === 2"
                  v-decorator="['supportStandard', {rules: [{required: true, message: '请输入'}]}]"
                >
                  <a-select-option value="0">无</a-select-option>
                </a-select>
              </a-form-item>
            ...
            >
              <a-form-item label="供养形式">
                <a-select
                  placeholder="请选择"
                  :disabled="openType === 2"
                  v-decorator="['supportForms', {rules: [{required: true, message: '请输入'}]}]"
                >
                  <a-select-option value="0">无</a-select-option>
                </a-select>
              </a-form-item>
...
import { editExtremelyPoorPeople } from '@/api/actualPopulation'
// 表单字段
const fields = [
  // 'description',
  'id',
      ...
      form.validateFields((errors, values) => {
        ...
        if (!errors) {
          ...
          if (data.approvalDate) {
            data.approvalDate = this.parseUtcTime(data.approvalDate) + ' 00:00:00'
          } else {
          ...
          return editExtremelyPoorPeople(data).then((res) => {
            // 重置表单数据
            form.resetFields()
            if (res.code === 200) {
              form.resetFields()
