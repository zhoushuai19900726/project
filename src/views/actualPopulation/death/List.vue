            ...
            <template v-if="advanced">
              ...
              >
                <a-form-item label="结婚时间">
                  <a-date-picker
                    v-model="queryParam.marriageDate"
                    style="width: 100%"
                    placeholder="请输入出生日期"
                  />
                </a-form-item>
...
export default {
  ...
  data () {
    ...
    return {
      ...
      loadData: (parameter) => {
        ...
        return getDeadPopulation(requestParameters).then((res) => {
          console.log(res)
          return res.result
        })
      },
  ...
  methods: {
    ...
    handleDel (record) {
      ...
      arr = [record.id]
      return deleteDeadPopulation(arr).then((res) => {
        console.log(res)
        if (res.code === 200) {
