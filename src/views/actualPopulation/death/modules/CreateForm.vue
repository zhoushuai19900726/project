        ...
        <a-card title="新增死亡人口">
          <a-row>
            ...
            <a-col
              ...
              :sm="24"
            >
              <a-form-item label="结婚时间">
                <a-date-picker
                  v-decorator="['marriageDate']"
                  placeholder="请输入"
                />
              </a-form-item>
            </a-col>
...
import { editDeadPopulation } from '@/api/actualPopulation'
// 表单字段
const fields = [
  // 'description',
  'id',
  ...
  methods: {
    ...
    submitSpecial () {
      ...
      form.validateFields((errors, values) => {
        ...
        if (!errors) {
          var data = { ...values }
          data.deathDate = this.parseUtcTime(data.deathDate) + ' 00:00:00'
          data.marriageDate = this.parseUtcTime(data.marriageDate) + ' 00:00:00'
          data.basicsId = that.id
          if (that.openType === 1) {
            console.log('我是修改')
            data.id = that.specialId
          }
          return editDeadPopulation(data).then((res) => {
            // 重置表单数据
            form.resetFields()
            if (res.code === 200) {
              form.resetFields()
