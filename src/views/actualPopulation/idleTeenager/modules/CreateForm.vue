            ...
            >
              <a-form-item label="是否有不良行为">
                <a-select
                  placeholder="请选择"
                  :disabled="openType === 2"
                  v-decorator="['badBehavior', {rules: [{required: true, message: '请输入'}]}]"
                >
            ...
            >
              <a-form-item label="是否有违法犯罪经历">
                <a-select
                  placeholder="请选择"
                  :disabled="openType === 2"
                  v-decorator="['criminalExperience']"
                >
                  <a-select-option value="0">是</a-select-option>
...
import { editIdleYouth } from '@/api/actualPopulation'
// 表单字段
const fields = [
  // 'description',
  'id',
    ...
    submitSpecial () {
      ...
      form.validateFields((errors, values) => {
        ...
        if (!errors) {
          ...
          return editIdleYouth(data).then((res) => {
            // 重置表单数据
            form.resetFields()
            if (res.code === 200) {
              form.resetFields()
