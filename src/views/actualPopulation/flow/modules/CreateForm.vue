          ...
          <a-row>
            ...
            >
              <a-form-item label="证件到期日期">
                <a-date-picker
                  v-decorator="['certificateExpirationDate']"
                  placeholder="请输入证件到期日期"
                />
              </a-form-item>
            </a-col>
            <a-col
...
import { editFloatingPopulation } from '@/api/actualPopulation'
// 表单字段
const fields = [
  // 'description',
  'id',
  ...
  methods: {
    ...
    submitSpecial () {
      ...
      form.validateFields((errors, values) => {
        ...
        if (!errors) {
          var data = { ...values }
          data.registrationDate = this.parseUtcTime(data.registrationDate) + ' 00:00:00'
          data.certificateExpirationDate = this.parseUtcTime(data.certificateExpirationDate) + ' 00:00:00'
          data.basicsId = that.id
          if (that.openType === 1) {
            console.log('我是修改')
            data.id = that.specialId
          }
          return editFloatingPopulation(data).then((res) => {
            // 重置表单数据
            form.resetFields()
            form1.resetFields()
            if (res.code === 200) {
