          ...
          <a-row>
            ...
            >
              <a-form-item label="残疾程度">
                <a-select
                  placeholder="请选择"
                  :disabled="openType === 2"
                  v-decorator="['disabilityDegree', {rules: [{required: true, message: '请输入'}]}]"
                >
                  <a-select-option value="0">无</a-select-option>
                </a-select>
              </a-form-item>
            ...
            >
              <a-form-item label="残疾类型">
                <a-select
                  placeholder="请选择"
                  :disabled="openType === 2"
                  v-decorator="['disabilityType', {rules: [{required: true, message: '请输入'}]}]"
                >
                  <a-select-option value="0">无</a-select-option>
                </a-select>
              </a-form-item>
            ...
            >
              <a-form-item label="评定医院">
                <a-input
                  v-decorator="['evaluationHospital']"
                  placeholder="请输入"
                />
              </a-form-item>
            </a-col>
            ...
            >
              <a-form-item label="是否有残疾证">
                <a-select
                  placeholder="请选择"
                  :disabled="openType === 2"
                  v-decorator="['disabilityCertificate']"
                >
            ...
            >
              <a-form-item label="领证时间">
                <a-date-picker
                  :disabled="openType === 2"
                  v-decorator="['obtainingCertificateDate']"
                  placeholder="请输入"
                />
              </a-form-item>
            </a-col>
...
import { editDisabledPersons } from '@/api/actualPopulation'
// 表单字段
const fields = [
  // 'description',
  'id',
  ...
  methods: {
    ...
    submitSpecial () {
      ...
      form.validateFields((errors, values) => {
        ...
        if (!errors) {
          var data = { ...values }
          data.obtainingCertificateDate = this.parseUtcTime(data.obtainingCertificateDate) + ' 00:00:00'
          data.basicsId = that.id
          if (that.openType === 1) {
            console.log('我是修改')
            data.id = that.specialId
          }
          return editDisabledPersons(data).then((res) => {
            // 重置表单数据
            form.resetFields()
            if (res.code === 200) {
              form.resetFields()
