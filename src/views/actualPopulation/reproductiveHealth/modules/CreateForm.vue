          ...
          <a-row>
            ...
            >
              <a-form-item label="生育证类型">
                <a-select
                  placeholder="请选择"
                  :disabled="openType === 2"
                  v-decorator="['birthCertificateType']"
                >
                  <a-select-option value="0">无</a-select-option>
                </a-select>
              </a-form-item>
            ...
            >
              <a-form-item label="政策生育类型">
                <a-select
                  placeholder="请选择"
                  :disabled="openType === 2"
                  v-decorator="['policyFertilityType']"
                >
                  <a-select-option value="0">无</a-select-option>
                </a-select>
              </a-form-item>
            ...
            >
              <a-form-item label="奖励救助类型">
                <a-select
                  placeholder="请选择"
                  :disabled="openType === 2"
                  v-decorator="['rewardAssistanceType']"
                >
                  <a-select-option value="0">无</a-select-option>
                </a-select>
              </a-form-item>
            ...
            >
              <a-form-item label="初婚上报日期">
                <a-date-picker
                  :disabled="openType === 2"
                  v-decorator="['firstMarriageReportDate']"
                  placeholder="请输入"
                />
              </a-form-item>
            </a-col>
            ...
            >
              <a-form-item label="生育指标状况">
                <a-input
                  v-decorator="['fertilityIndicators']"
                  placeholder="请输入"
                />
              </a-form-item>
            </a-col>
...
import { editReproductive } from '@/api/actualPopulation'
// 表单字段
const fields = [
  // 'description',
  'id',
...
const field = [
  ...
  'birthControlType',
  'policyFertilityType',
  'rewardAssistanceType',
  'rewardAmount',
  'firstMarriageDate',
  'firstMarriageReportDate',
  'birthsNumber',
  'raiseRelationship',
  'birthReportDate',
  'fertilityIndicators',
    ...
    submitSpecial () {
      ...
      form.validateFields((errors, values) => {
        ...
        if (!errors) {
          ...
          data.firstMarriageDate = this.parseUtcTime(data.firstMarriageDate) + ' 00:00:00'
          data.firstMarriageReportDate = this.parseUtcTime(data.firstMarriageReportDate) + ' 00:00:00'
          data.birthReportDate = this.parseUtcTime(data.birthReportDate) + ' 00:00:00'
          if (that.openType === 1) {
            console.log('我是修改')
            data.id = that.specialId
          }
          return editReproductive(data).then((res) => {
            // 重置表单数据
            form.resetFields()
            if (res.code === 200) {
              form.resetFields()
