...
import { getftBehindPopulation, deleteftBehindPopulation } from '@/api/actualPopulation'
...
export default {
  ...
  data () {
    ...
    return {
      ...
      loadData: (parameter) => {
        ...
        return getftBehindPopulation(requestParameters).then((res) => {
          console.log(res)
          return res.result
        })
      },
  ...
  methods: {
    ...
    handleDel (record) {
      ...
      var id = record.id
      var arr = [id]
      return deleteftBehindPopulation(arr).then((res) => {
        console.log(res)
        if (res.code === 200) {
