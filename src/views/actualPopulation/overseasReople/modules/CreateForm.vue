          ...
          <a-row>
            ...
            >
              <a-form-item label="预计离开日期">
                <a-date-picker
                  v-decorator="['expectedDepartureDate']"
                  placeholder="请输入"
                />
              </a-form-item>
            </a-col>
...
import { editOverseasPersonnel } from '@/api/actualPopulation'
// 表单字段
const fields = [
  // 'description',
  'id',
  ...
  methods: {
    ...
    submitSpecial () {
      ...
      form.validateFields((errors, values) => {
        ...
        if (!errors) {
          var data = { ...values }
          data.arrivalDate = this.parseUtcTime(data.arrivalDate) + ' 00:00:00'
          data.expectedDepartureDate = this.parseUtcTime(data.expectedDepartureDate) + ' 00:00:00'
          data.basicsId = that.id
          if (that.openType === 1) {
            console.log('我是修改')
            data.id = that.specialId
          }
          return editOverseasPersonnel(data).then((res) => {
            // 重置表单数据
            form.resetFields()
            if (res.code === 200) {
              form.resetFields()
